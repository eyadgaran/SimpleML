...
def assert_data_container_equal(a: Any, b: Any):
    ...
    elif isinstance(a, (list, int, dict, str, type(None))) or isinstance(
        b, (list, int, dict, str, type(None))
    ):
        assert a == b
    else:
...
def assert_split_equal(
    a: Union[Split, ProjectedDatasetSplit], b: Union[Split, ProjectedDatasetSplit]
) -> None:
    """
