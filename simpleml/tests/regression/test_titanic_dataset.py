...
class TitanicRegressionTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        """
        cls.db = Database(
            uri=f"sqlite:///{join(DATABASES_PATH, 'titanic-regression.db')}"
        )
        cls.db.initialize()
    ...
    def test_dask_datasets(self):
        ...
        for save_pattern in ["dask_disk_json", "dask_disk_csv", "dask_disk_parquet"]:
            with self.subTest(
                "Dask Dataset Regression with Titanic", save_pattern=save_pattern
            ):
                save_patterns["dataset"] = [save_pattern]
    ...
    def test_pandas_datasets(self):
        ...
        for save_pattern in [
            ...
            with self.subTest(
                "Pandas Dataset Regression with Titanic", save_pattern=save_pattern
            ):
                save_patterns["dataset"] = [save_pattern]
                regression_dataset = self.get_regression_artifact(
                    "dataset", name=name.format(test_name=save_pattern)
