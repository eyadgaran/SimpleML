...
class _DatasetTestHelper(object):
    ...
    def test_missing_split_column_error(self):
        ...
        """
        dataset = self.dummy_dataset
        dataset._external_file = dataset._external_file.drop(
            DATAFRAME_SPLIT_COLUMN, axis=1
        )
        with self.assertRaises(DatasetError):
...
class BasePandasDatasetTests(unittest.TestCase, _DatasetTestHelper):
    ...
    def expected_y(self):
        ...
        """
        return pd.DataFrame(
            [{"label": 3}, {"label": 33}, {"label": 333}], index=[10, 20, 30]
        )
...
class SingleLabelPandasDatasetTests(BasePandasDatasetTests):
    ...
    def expected_y(self):
        ...
        Y values are expected to be squeezed
        """
        return pd.Series([3, 33, 333], name="label", index=[10, 20, 30])
...
class MultiLabelPandasDatasetTests(BasePandasDatasetTests):
        ...
        return pd.DataFrame(
            [
                ...
                },
                {
                    ...
                    "label1": 33,
                    "label2": 44,
                    DATAFRAME_SPLIT_COLUMN: "VALIDATION",
                },
                {
                    "a": 111,
                    "b": 222,
    ...
    def dummy_dataset(self):
        dataset = self.dataset_cls(
            label_columns=["label1", "label2"], sqeeze_return=False
        )
        dataset.dataframe = self._data
        return dataset
...
class CSVFileBasedPandasDatasetTests(SingleLabelPandasDatasetTests):
    ...
    def dummy_dataset(self):
        dataset = self.dataset_cls(
            filepath="", format="csv", label_columns=["label"], squeeze_return=True
        )
        dataset.dataframe = self._data
        return dataset
...
class JSONFileBasedPandasDatasetTests(SingleLabelPandasDatasetTests):
    ...
    def dummy_dataset(self):
        dataset = self.dataset_cls(
            filepath="", format="json", label_columns=["label"], squeeze_return=True
        )
        dataset.dataframe = self._data
        return dataset
...
class ParquetFileBasedPandasDatasetTests(SingleLabelPandasDatasetTests):
    ...
    def dummy_dataset(self):
        dataset = self.dataset_cls(
            filepath="", format="parquet", label_columns=["label"], squeeze_return=True
        )
        dataset.dataframe = self._data
        return dataset
