...
class Registry(object):
    ...
    def register(self, cls: Type) -> None:
        ...
        if cls.__name__ in self.registry and cls is not self.registry[cls.__name__]:
            raise ValueError(
                "Cannot duplicate class in registry: {}".format(cls.__name__)
            )
    ...
    def context_register(self, cls: Type) -> None:
        ...
        if name in self.registry:
            LOGGER.debug(
                f"Overwriting registry key {name} for the scope of this context"
            )
    ...
    def get_from_registry(self, class_name: str) -> Optional[Type]:
        ...
        if cls is None:
            LOGGER.error(
                "Class not found for {}. Make sure to import the class into the registry before calling".format(
                    class_name
                )
            )
        return cls
