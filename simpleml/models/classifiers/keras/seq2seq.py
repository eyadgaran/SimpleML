...
class KerasSeq2SeqClassifier(KerasModelClassifier):
    ...
    def check_for_models(self, rebuild=False):
        if not hasattr(self, "inference_model"):
            self.inference_model = self.build_inference_network(
                self.external_model.__class__
            )
            self.transfer_weights(
                new_model=self.inference_model, old_model=self.external_model
            )
        elif rebuild:
            self.transfer_weights(
                new_model=self.inference_model, old_model=self.external_model
        ...
        if isinstance(X, (pd.DataFrame, pd.Series)):
            for index in X.index:
                transformed = self.pipeline.transform(
                    X.loc[index].to_frame().transpose(), **kwargs
                )
...
class KerasEncoderDecoderClassifier(KerasSeq2SeqClassifier):
    ...
    def check_for_models(self, rebuild=False):
        if not hasattr(self, "encoder_model") or not hasattr(self, "decoder_model"):
            self.encoder_model, self.decoder_model = self.build_inference_network(
                self.external_model.__class__
            )
            self.transfer_weights(
                new_model=self.encoder_model, old_model=self.external_model
            )
            self.transfer_weights(
                new_model=self.decoder_model, old_model=self.external_model
            )
        elif rebuild:
            self.transfer_weights(
                new_model=self.encoder_model, old_model=self.external_model
...
class KerasEncoderDecoderStateClassifier(KerasEncoderDecoderClassifier):
    ...
    def _predict(self, X, end_index, max_length=None, **kwargs):
        ...
        if not isinstance(
            decoder_states, list
        ):  # GRU only returns single state vs lstm h, c
            decoder_states = [decoder_states]

        while True:
            decoder_output_and_states = self.decode(
                [predicted_sequence] + decoder_states
            )
            decoder_prediction = decoder_output_and_states[0]
            decoder_states = decoder_output_and_states[
                1:
            ]  # Multiple for lstm, single for gru
            ...
            predicted_sequence = np.concatenate(
                [predicted_sequence, next_tokens], axis=axis
            )

            # Exit conditions
            # TODO: generic support for different length sequences
            if (next_tokens == end_index).any() or (
                max_length is not None and predicted_sequence.shape[1] >= max_length
            ):
                break
...
class KerasEncoderDecoderStatelessClassifier(KerasEncoderDecoderStateClassifier):
    ...
    def _predict(self, X, end_index, max_length=None, **kwargs):
        ...
        while True:
            ...
            # TODO: generic support for different length sequences
            if (next_tokens == end_index).any() or (
                max_length is not None and predicted_sequence.shape[axis] >= max_length
            ):
                break
