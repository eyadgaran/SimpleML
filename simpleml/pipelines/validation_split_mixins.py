...
class ExplicitSplitMixin(SplitMixin):
    def split_dataset(self) -> None:
        ...
        self._dataset_splits = self.containerize_split(
            {
                split_name: IdentityProjectedDatasetSplit(
                    dataset=self.dataset, split=split_name
                )
...
class RandomSplitMixin(SplitMixin):
    ...
    **Redefines splits so custom named splits in dataset cannot be referenced
    ...
    def split_dataset(self) -> None:
        ...
        self._dataset_splits = self.containerize_split(
            {
                TRAIN_SPLIT: IndexBasedProjectedDatasetSplit(
                    dataset=self.dataset, split=None, indices=train_indices
                ),
                VALIDATION_SPLIT: IndexBasedProjectedDatasetSplit(
                    dataset=self.dataset, split=None, indices=validation_indices
                ),
                TEST_SPLIT: IndexBasedProjectedDatasetSplit(
                    dataset=self.dataset, split=None, indices=test_indices
                ),
            }
        )
