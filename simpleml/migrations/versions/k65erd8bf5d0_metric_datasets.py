...
class DatasetModel(UpgradeTableModel):
    ...
    pipeline_id = Column(
        GUID, ForeignKey("pipelines.id", name="datasets_pipeline_id_fkey")
    )
    pipeline = relationship(
        lambda: PipelineModel, enable_typechecks=False, foreign_keys=[pipeline_id]
    )
...
class PipelineModel(UpgradeTableModel):
    ...
    dataset = relationship(
        lambda: DatasetModel, enable_typechecks=False, foreign_keys=[dataset_id]
    )
...
def upgrade_hash(record):
    ...
    if record.name.startswith("in_sample_"):
        ...
        SIMPLEML_REGISTRY.get(record.model.pipeline.registered_name),
        (ExplicitSplitMixin, RandomSplitMixin),
    ):
        dataset_split = "TEST"
    else:
        dataset_split = None
