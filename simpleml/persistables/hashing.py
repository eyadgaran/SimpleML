...
def _pandas_hash(df):
    ...
    return hash_pandas_object(
        df.applymap(
            lambda element: CustomHasherMixin.custom_hasher(element)
...
class CustomHasherMixin(object):
    ...
    def custom_hasher(cls, object_to_hash: Any) -> str:
        ...
        if isinstance(object_to_hash, (set, tuple, list)):
            ...
            hash_output = int(
                object_to_hash.map_partitions(_pandas_hash, meta=(None, int))
                .sum()
                .compute()
            )
            ...
            hash_output = int(
                hash_pandas_object(
                    object_to_hash.apply(
                        lambda element: cls.custom_hasher(element)
                        if isinstance(element, np.ndarray)
                        else element
                    ),
                    index=False,
            ...
            LOGGER.warning(
                f"Hashing class import path for {object_to_hash}, if a fully qualified import path is not used, calling again from a different location will yield different results!"
            )
            hash_output = cls.custom_hasher(
                f"{object_to_hash.__module__}.{object_to_hash.__name__}"
            )
            ...
            hash_output = cls.custom_hasher(
                (object_to_hash.__class__, object_to_hash.__dict__)
            )
            ...
            LOGGER.warning(
                f"Unable to find suitable representation of {object_to_hash}, passing through to hash function directly. (This may result in future breaking changes)"
            )
        ...
        if isinstance(object_to_hash, tuple):
            for i in object_to_hash:
                if not isinstance(i, (float, str, int)):
                    raise ValueError(
                        f"md5 can only hash simple dtypes, passed: {type(i)}"
                    )
        elif not isinstance(object_to_hash, (float, str, int)):
            raise ValueError(
                f"md5 can only hash simple dtypes, passed: {type(object_to_hash)}"
            )
